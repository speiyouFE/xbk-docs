...
export default {
  ...
  computed: {
    dropdownSize() {
      return this.size || (this.$$TAL_SC || {}).size
    }
  },
  watch: {
    visible(val) {
      this.broadcast('ScDropdownMenu', 'visible', val)
      this.$emit('visible-change', val)
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector('.sc-dropdown-selfdefine')
      if (selfDefine) {
        // 自定义
        if (val) {
  ...
  methods: {
    ...
    initAria() {
      ...
      if (!this.splitButton) {
        ...
        this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' sc-dropdown-selfdefine') // 控制
      }
    },
    initEvent() {
    ...
    handleMenuItemClick(command, instance) {
      ...
      this.$emit('change', command, instance)
    },
    triggerElmFocus() {
  ...
  render() {
    ...
    // ) : (
    //   <el-button-group>
    //     <sc-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick}>
    //       {this.$slots.default}
    //     </sc-button>
    ...
    // )
    return (
      <div class="sc-dropdown" v-clickoutside={hide}>
        {triggerElm}
        {this.$slots.dropdown}
      </div>
    )
